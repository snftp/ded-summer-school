...
int Solutions(double a, double b, double c) { // NOTE: is it a good idea to have separate p0 for 1 root case? NO IF REPLACING ROOTS WITH ARRAY // TODO: maybe a better name for p0, p1, etc ROOTS? Possibly use an array, double roots[2] YES?
    ...
    if (fabs(a) <= DBL_EPSILON) { // TODO: beware of the indentation DONE
            if (fabs(b) > DBL_EPSILON) {
                ...
                if (fabs(c) <= DBL_EPSILON) {
                    roots[0] = -1 // INFINITY NUMBER OF ROOTS
                    roots[1] = -1
